...
\markboth
{B. Gautam and A. R. Subedi: Deflation and Measurement Limits in Variational Quantum Modal Analysis}
{B. Gautam and A. R. Subedi: Deflation and Measurement Limits in Variational Quantum Modal Analysis}
...
\begin{table}[t]
\centering
\caption{End-to-end asymptotic time and space for the $k$ lowest modes of an $N$-DOF banded 1D chain (constant half-bandwidth $b$). Dashes mark steps the classical solver does not perform.}
...
\vspace{0.35em}
\parbox{\columnwidth}{\footnotesize $T_{\mathrm{it}}$ is the Lanczos iteration count: approximately constant with shift-invert ARPACK, and $O(\sqrt N)$--$O(N)$ for unpreconditioned Lanczos depending on the relative spectral gap. $^{\dagger}$$O(N)$ statevector amplitudes plus $O(N^2)$ classical storage for the dense-whitened operator and its Pauli list.}
\end{table}
