...
- 22 TQE-formatted pages (IEEEtran, US letter), plus a 1-page supplementary document.
  The canonical build is now IEEEtran: building `Qubits_Are_Not_Enough_TQE.tex` with no
  flags produces the submitted PDF. See `PDF_PREFLIGHT.txt`, which is regenerated by
  `make_preflight.py` and reports pages, fonts, Type-3 count and embedding for both files.
